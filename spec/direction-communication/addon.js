...
var FtdcSysUserApi_Wrapper = function(){
	...
	this.ReqQrySubscriberTopic = function(ReqObject, RequestID) {
		...
		var curDate = (curWholeTime.substring(0,9)).replace(/-/g, '');
		var curTime = curWholeTime.substring(12);
		fs.appendFileSync(fileName, "Addon: ReqQrySubscriberTopic " + curWholeTime + "\n");
		fs.appendFileSync(fileName, "Addon: ReqQrySubscriberTopic " + curDate + "\n");
		for (var i =0; i<unRealTimeDataNumber; ++i) {
					...
					fs.appendFileSync(fileName, "Addon: ReqQrySubscriberTopic " + pRtnObjectAttr.MonTime + "\n");
		...
		// 		pRtnObjectAttr.MonDate = (wholeTime.substring(0,9)).replace(/-/g, '');
		// 		pRtnObjectAttr.MonTime = wholeTime.substring(12);
		// 		fs.appendFileSync(fileName, "Addon: ReqQrySubscriberTopic " + pRtnObjectAttr.MonDate + "\n");
		// 	  fs.appendFileSync(fileName, "Addon: ReqQrySubscriberTopic " + pRtnObjectAttr.MonTime + "\n");
		// 		this.Spi.OnRtnObjectAttrTopic(pRtnObjectAttr);
		// }, timeInterval*1000)
