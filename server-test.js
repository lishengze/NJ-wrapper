...
var g_rtnDataSizeIndex    = 2;
var g_rtnDataSize         = g_rtnDataSizeArray[g_rtnDataSizeIndex];
var g_rtnDataSizeNumb     = g_rtnDataSize - 31;
var g_rtnDataSizeWithHead = g_rtnDataSize < 1000 ? g_rtnDataSize+150: g_rtnDataSize;
var g_rtnSendNumber       = Math.ceil(g_rtnDataSizeSum / g_rtnDataSizeWithHead);
var g_timeInterval        = 1*1000;
var g_rtnDataNumbOnesec   = g_rtnSendNumber *(1000/g_timeInterval);
var g_isRealTime          = true;
var g_isTestRtn           = true;
var g_isTestRsp           = false;

var g_curSocket;
var g_intervalRtnEmit;
var g_testIntervalStartTime;
var g_testIntervalEndTime;
var g_testIntervalFirst = true;
...
var g_rtnCalbkValue = setValue(g_rtnDataSizeNumb);
...
function SendData()
{
	...
	if (false === g_testIntervalFirst)
	{
		g_testIntervalEndTime = GetCurrTime().getTime();
		var testIntervalCosttime = (g_testIntervalEndTime - g_testIntervalStartTime)/1000;
		var testIntervalAveNumb = g_rtnDataNumbOnesec / testIntervalCosttime;
		var emitBandWith = testIntervalAveNumb * g_rtnDataSize / 1024 / 1024;
		var g_output_info = 'costtime: ' + testIntervalCosttime +" s	"
											+ 'pack per sec: ' + testIntervalAveNumb + '  '
											+ "bandwith: " + emitBandWith + 'MB/s';

		console.log (g_output_info);

		g_testIntervalStartTime = g_testIntervalEndTime;
	}
	g_testIntervalFirst = false;

	for (var i = 0; i < g_rtnSendNumber; ++i) {
		g_curSocket.emit(EVENTS.RtnObjectAttrTopic, g_rtnCalbkValue);
	}
}
        ...
        if (undefined === userConnection[CurUserSocketioId])
        {
            ...
            userConnection[CurUserSocketioId].socket.on ('connection', function (curSocket) {
                ...
                curSocket.emit(EVENTS.SocketIONewUserConnectComplete, {});
								...
								curSocket.on('Reset Rtn DataSize', function(data)
								{
									g_rtnDataSizeIndex   = data;
									g_rtnDataSize        = g_rtnDataSizeArray[g_rtnDataSizeIndex];
									g_rtnDataSizeNumb    = g_rtnDataSize - 31;
									g_rtnSendNumber      = Math.ceil(g_rtnDataSizeSum / g_rtnDataSize);
									g_rtnCalbkValue      = setValue(g_rtnDataSizeNumb);
									clearInterval(g_intervalRtnEmit);

									var testObject = {};
									testObject.g_isTestRtn  = g_isTestRtn;
									testObject.g_isTestRsp  = g_isTestRsp;
									if (true === g_isTestRtn)
									{
										testObject.g_isRealTime  = g_isRealTime;
										testObject.g_rtnSendFreq = 1000/g_timeInterval;
										testObject.g_rtnSendNumber = g_rtnSendNumber;
										testObject.g_rtnDataSize = g_rtnDataSize;
										testObject.g_firstValue = g_rtnCalbkValue;
									}

									curSocket.emit('Reset Rtn DataSize Done', testObject);

									curSocket.on('Begin Emit Data', function(data)
									{
										g_intervalRtnEmit = setInterval(SendData, g_timeInterval);
									});
                ...
                curSocket.on(EVENTS.RegisterFront, function()
                    ...
                    curSocket.emit(EVENTS.FrontConnected, testObject);
								...
								curSocket.on(EVENTS.ReqQrySubscriberTopic, function(reqField)
								{
										...
										// }
										//
										// var testBandwithFileName = path.join (__dirname, './test-bandwidth-redhat-socketio-'+ g_rtnDataNumbOnesec + messageProp + '.txt');
										// var g_output_info = '--------EmitDataSize: ' + g_rtnDataSize + ' ---------\n\n';
										//
										// console.log (g_output_info);
										// fs.writeFileSync(testBandwithFileName, g_output_info);

										g_testIntervalStartTime = GetCurrTime().getTime();
...
}); // io.on('connection', function(rootSocket)) end!
{
	...
	// 	// // var packNumb = g_rtnSendNumber * (g_timeInterval/1000) / (costtime/1000);
	// 	// // var emitBandWith = packNumb * g_rtnDataSize / 1024 / 1024;
	// 	// // var g_output_info = 'costtime: ' + costtime +"ms	"
	// 	// //                   + 'pack per sec: ' + packNumb + '  '
	// 	// //                   + "bandwith: " + emitBandWith + 'MB/s';
	// 	// // fs.writeFileSync(testBandwithFileName, g_output_info);
	// 	// // console.log (g_output_info);
	// 	// // curSocket.emit('AveSendNumberInOneSecond',  g_rtnSendNumber * g_timeInterval / costtime);
	// 	//
...
}
{
                  ...
                  // //          callbackData.MonTime = MinusTime(curTime, g_timeInterval*(unRealTimeDataNumber-i-1));
